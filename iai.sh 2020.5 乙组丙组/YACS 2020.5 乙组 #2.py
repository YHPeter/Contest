...
题目描述
给定一个正整数 nn，表示人口数量。一开始，每个居民拥有的财产记为 a_1, a_2, \cdots, a_na 
1
​	
 ,a 
2
...
输入格式
第一行：两个正整数 nn 和 mm；
第二行：nn个正整数表示 a_1, a_2, \cdots, a_na 
1
​	
 ,a 
2
...
​	
 ；
接下来 mm 行：每行依次表示一次财产变化：

以字母 i 开头的表示一次个体变化，后接两个整数参数 uu 和 tt，uu 表示财产发生变化的居民编号，tt 表示财产变化的结果；
以字母 p 开头的表示一次政策变化，后接一个整数参数 ss，ss 表示政策的标准线。
输出格式
共 nn 行：第 ii 行有一个整数，表示 ii 号居民最后拥有的财产数量。

数据范围
1\leq s\leq 1000001≤s≤100000；
1\leq t\leq 1000001≤t≤100000；
1\leq a_i\leq 1000001≤a 
i
​	
 ≤100000；
对于 30\%30% 的数据，n\leq 1000n≤1000，m\leq 1000m≤1000；
对于 60\%60% 的数据，n\leq 10000n≤10000，m\leq 10000m≤10000；
对于 100\%100% 的数据，1\leq n\leq 1000001≤n≤100000，1\leq m\leq 1000001≤m≤100000。
样例数据
输入:
4 3
10 20 30 40
